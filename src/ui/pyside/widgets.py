...
Professional-grade UI components using QtAwesome icons.
"""
...
class IconKPICard(QFrame):
    """
    Stylish KPI card with an icon, title, and value.
    Accepts a QIcon (normally from QtAwesome) – no emoji strings.
    """
    def __init__(self, title: str, value: str, icon: QIcon, color: str, parent=None):
        ...
        self.val_lbl.setStyleSheet(
            f"font-size: 22px; font-weight: bold; color: {color}; background: transparent; border: none;"
        )
        self.val_lbl.setWordWrap(False) # Prevent vertical breaking, prefer horizontal expansion
    ...
    def set_color(self, color: str):
        """Change the accent colour dynamically (e.g., for low‑stock alert)."""
        self.color = color
        self.val_lbl.setStyleSheet(
            f"font-size: 24px; font-weight: bold; color: {color}; background: transparent; border: none;"
        )
        self.setStyleSheet(f"""
            IconKPICard {{
                background-color: {Theme.BG_SECONDARY};
...
class StatusBar(QFrame):
    """Application status bar with user info, time, and DB status – all icons from QtAwesome."""
    ...
    def __init__(self, parent=None):
        ...
        self.status_icon.setPixmap(qta_icon('fa5s.circle', color=Theme.SUCCESS).pixmap(8, 8))
        self.status_lbl = QLabel("Ready")
        self.status_lbl.setStyleSheet(f"font-size: 11px; color: {Theme.SUCCESS}; border: none; background: transparent;")

        # DB type
        self.db_icon = QLabel()
        self.db_icon.setPixmap(qta_icon('fa5s.database', color=Theme.TEXT_MUTED).pixmap(12, 12))
        self.db_lbl = QLabel("SQLite")
        self.db_lbl.setStyleSheet(lbl_style)

        # User
        self.user_icon = QLabel()
        self.user_icon.setPixmap(qta_icon('fa5s.user', color=Theme.TEXT_MUTED).pixmap(12, 12))
        self.user_lbl = QLabel("Admin")
        self.user_lbl.setStyleSheet(lbl_style)

        # Time
        self.time_icon = QLabel()
        self.time_icon.setPixmap(qta_icon('fa5s.clock', color=Theme.TEXT_MUTED).pixmap(12, 12))
    ...
    def set_status(self, msg: str, color: str = None):
        ...
        self.status_icon.setPixmap(qta_icon('fa5s.circle', color=color).pixmap(8, 8))
...
class EmptyState(QWidget):
    """Placeholder widget shown when a table or list has no data. Now uses QIcon."""

    def __init__(self, icon: QIcon = None, title: str = "", subtitle: str = "", parent=None):
        super().__init__(parent)
        layout = QVBoxLayout(self)
...
class ProductFormDialog(QDialog):
    """Modal dialog for creating or editing a product. No icons needed."""
...
class ConfirmDialog(QDialog):
    """Professional confirmation dialog. Button colours by role."""
    ...
    def __init__(self, title: str, message: str, details: str = "",
        ...
        if confirm_color == "danger":
            style = Theme.btn_danger()
        elif confirm_color == "success":
            style = Theme.btn_success()
        else:
